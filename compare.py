...
given_names_mode = single_mode = False
original_path = new_path = ""

for arg in range(0, len(argv)):
    if arg == "-op":
        original_path = argv[arg+1]
        given_names_mode = True
    if arg == "-np":
        new_path = argv[arg+1]
        given_names_mode = True
    if arg == "-s":
...
if not single_mode:
    ...
    directory1 = original_path if original_path else os.getcwd()
    directory2 = new_path if new_path else f"{directory1}/Reencode/"
    ...
    size1 = round(os.path.getsize(original_path)/1024/1024)
    size2 = round(os.path.getsize(new_path)/1024/1024)
    total_start_size = size1
    total_end_size = size2
...
if total_end_size > 974:
    ...
    size_char2 = "G"
print(f"{'TOTAL ' if not single_mode else ''}Original: {total_start_size:.2f} {size_char1}iB, Reencoded: {total_end_size:.2f} {size_char2}iB, reduction: {percentage_difference:.2f}%")