...
rename = show = overwrite_mode = movie = art_path = lang = ""
q = -1
fflvl = "4.1"
subenc = "copy"
year = season = cq = subtitle_input = ep_i = start_i = end_i = index_checks = ep = 0
ruler_2nd = pretty = external_subtitles = replace_periods = skip_subtitles = skip_reencode = has_art = b10_mode = tune_animation = tune_film = gpu = False
...
for arg in range(0, len(argv)):
    ...
    elif argv[arg] == "-tf": tune_film = True
...
for item in os.listdir(cwd):
    ...
    str_tune = f" -tune {'animation' if tune_animation else ('film' if tune_film else '')}"
    ...
    if external_subtitles: cmd = f"{cmd} -i \"./{item_title}.{lang}.srt\""
    if skip_reencode:
        cmd = f"{cmd} -map 0:v -gpu 0 -c:v:0 copy"
    else:
        str_subtitlemapping = f' -map ' + ('2' if has_art else '1') if external_subtitles else ''
    ...
    if not skip_subtitles:
        ...
        if external_subtitles:
            cmd = f"{cmd} -metadata:s:s:0 language={lang}"
